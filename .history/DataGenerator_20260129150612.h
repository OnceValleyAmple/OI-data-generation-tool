...
namespace FESDRER_RAND {
    ...
    */
    inline long long random( long long l , long long r , double opt = 1.0 ) {
        ...
        long long res = l + ( long long ) ( ( r - l + 1 ) * y );
        if ( res > r ) res = r;
    ...
    */
    inline double randomreal( double l , double r , double opt = 1.0 ) {
        ...
        // 2. 均匀分布特判（最快路径）
        // 如果 opt 接近 1，直接利用标准库生成 [l, r) 的实数
        if ( std::abs( opt - 1.0 ) <= 1e-8 ) {
            return std::uniform_real_distribution < double >( l , r )( rng_64 );
        }

        // 3. 高精度基准随机数生成
        // 直接生成 [0.0, 1.0) 的双精度浮点数
        // 相比原代码的 (1~10000)/10000，这里利用了 double 的全部 53 位精度
        static std::uniform_real_distribution < double > dist( 0.0 , 1.0 );
        double x = dist( rng_64 );

        // 4. 应用形状函数
        // 核心公式：y = (1 - (1-x)^opt)^(1/opt)
        // 效果：opt > 1 时，曲线向 1 膨胀；opt < 1 时，曲线向 0 塌缩
        double y = std::pow( 1.0 - std::pow( 1.0 - x , opt ) , 1.0 / opt );

        // 5. 线性映射到目标区间
        // 实数映射不需要像整数那样 +1，直接缩放即可
        return ( r - l ) * y + l;
    }
        ...
        long long getScale( int index , bool randomize = false , double opt = 1.0 ) const {
            ...
            for ( const auto &g : gradients_ ) {
                if ( index <= cumulative + g.fileCount ) {
                    ...
                    if ( randomize ) {
                        return FESDRER_RAND::random( subMin , subMax , opt );
                    } else {
                        // 确定性：取子区间中点或线性插值
