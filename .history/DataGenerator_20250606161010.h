...
namespace FESDRER_RAND {
    ...
    */
    ...
    public:
        ...
        //随机生成一个数组。
        template < typename T >
        inline std::vector < T > random_sequence( int n , T l , T r ) {
            std::vector < T > ret;
            ret.clear();
        ...
        inline void random_sequence( T first , T last , V l , V r ) {
    ...
    public:
        //输出数组，输出一行。
        template < typename T >
        inline void print_sequence( T first , T second ) {
        ...
        //输出数组，输出一行。
        template < typename T >
        inline void print_sequence( std::vector < T > s ) {
        ...
        //输出由二元组组成的数组，每个二元组一行。
        template < typename T >
        inline void print_sequence_pair( T first , T second ) {
            for ( T it = first ; it != second ; it++ ) std::cout << ( * it ).first << " " << ( * it ).second << '\n';
        }

        //输出由二元组组成的数组，每个二元组一行。
        template < typename T >
        inline void print_sequence_pair( std::vector < T > s ) {
            for ( T i : s ) std::cout << i.first << " " << i.second << '\n';
        }

        //输出二维数组，每个维度的数组一行。
        template < typename T >
        inline void print_sequence_of_sequence( std::vector < std::vector < T>> s ) {
        ...
        */
        inline void print_graph( std::vector < std::pair < std::pair < int , int > , int>> graph ) {
            for ( std::pair < std::pair < int , int > , int > i : graph )
    ...
    public:
        ...
        template < typename T >
        inline std::vector < T > random_different( T l , T r , int p ) {
            assert( r - l + 1 >= p );
        ...
        //返回 [l,r] 中除了 S 中元素外的 p 个不同的元素。不调用 S 代表 S 为空。
        template < typename T >
        inline std::vector < T > random_different( T l , T r , int p , std::unordered_set < T > S ) {
        ...
        random_divide_num( int n , int p , std::vector < int > limits = std::vector < int >( 0 ) ) {
    ...
    public:
        ...
        get_outedge( int n , std::vector < std::pair < int , int>> edges , bool directional ) {
            std::vector < std::vector < int>> e( n , std::vector < int >( 0 ) );
        ...
        */
        template < typename __Rnm , typename __Rgraph >
    ...
    public:
        ...
        */
        inline std::vector < std::pair < int , int>>
        random_binary_tree( int n , int opt = 50 , double rho = 1 , bool vertices_rand = 1 ) {
    ...
    public:
        ...
        */
        inline std::vector < int >
        nm( int N , int M , bool connected , bool repeated_edges , bool self_rings , bool directional ,
            double opt = 1 ) {
            ...
            if ( !directional && !repeated_edges )
                Mr = std::min( 1ll * Mr , 1ll * n * ( n - 1 ) / 2 + n * ( self_rings ) );
            if ( Ml > Mr ) return { -1 , -1 };
            m = random( Ml , Mr , opt );
        ...
        */
        inline std::vector < std::pair < int , int>>
        graph( int n , int m , bool connected , bool repeated_edges , bool self_rings , bool directional ,
                ...
                for ( int x = 1 ; x <= nn ; ++x )
                    for ( int y = 1 ; y <= mm ; ++y ) {
                        if ( x < nn ) {
                            ...
                            if ( y < mm )
                                graph.push_back( { { gid( x , y ) , gid( x + 1 , y + 1 ) } ,
                                                   random( std::min( l + 3 , r ) , r ) } );
                        }
                        if ( y < mm )
                            graph.push_back(
                                    { { gid( x , y ) , gid( x , y + 1 ) } , random( std::min( l + 3 , r ) , r ) } );
                    }
    ...
    public:
        ...
        inline std::vector < std::pair < int , int>> dag( int n , int m , bool single_s , bool single_t ) {
            ...
            if ( single_s && !single_t ) graph = RandomGraph.graph( n , m , 1 , 1 , 0 , 0 );
                ...
                std::vector < std::vector < int>> e = RandomGraphTool.get_outedge( n , RandomTree.random_tree( n , 5 ) ,
                                                                                   0 );
                std::vector < int > start = RandomAnother.random_different( 1 , n , random( 1 , n , 0.3 ) ) , vis( n ,
                                                                                                                   0 ) , topo(
                        0 );
                std::queue < int > q;
                while ( q.size() ) q.pop();
    ...
    public:
        ...
        */
        inline std::vector < std::pair < int , int>>
        scc( int n , int m , int sccn , int sccm , std::vector < std::pair < int , int>> dag , double rho = 1 ) {
            ...
            for ( std::pair < int , int > i : dag )
                graph.push_back( { Get( scc[i.first - 1] ) , Get( scc[i.second - 1] ) } );
            RandomGraphTool.random_vertices( n , graph );
            return graph;
        }
    };
    ...
    public:
        ...
        */
        inline std::vector < std::pair < int , int>>
        edcc( int n , int m , int edccn , std::vector < std::pair < int , int>> tree , double rho = 1 ) {
    ...
    public:
        ...
        */
        inline std::vector < int > nm( int N , int M , double opt = 1 , double rhovdccn = 0.4 ) {
            ...
            int n = random( 3 , std::min( N , M ) , opt ) , vdccn = ( M == n - 1 ? n - 1 : std::max( 1 , std::min(
                    int( n * rhovdccn ) , n - 1 ) ) );
...
namespace FESDRER_CHECK {
    ...
    public:
        ...
        inline std::vector < std::vector < int>> check( int n , std::vector < std::pair < int , int>> graph ) {
            ...
            for ( int i = 1 ; i <= n ; i++ )
                if ( !dfn[i] )
                    tarjan( i , dfn , low , stk , fst , nxt , tal , instk , scc , tod );
            return scc;
        }
    };
    ...
    private:
        inline void
        tarjan( int x , int in_edge , std::vector < int > &fst , std::vector < int > &nxt , std::vector < int > &tal ,
    ...
    public:
        ...
        inline void check( int n , std::vector < std::pair < int , int>> graph ) {
            ...
            for ( int i = 1 ; i <= n ; i++ )
                if ( !dfn[i] )
                    tarjan( i , 0 , fst , nxt , tal , dfn , low , stk , edcc , tod );
        }
    };
    ...
    public:
        ...
        inline void check( int n , std::vector < std::pair < int , int>> graph ) {
            ...
            for ( int i = 1 ; i <= n ; i++ )
                if ( !dfn[i] )
                    tarjan( ( root = i ) , fst , nxt , tal , dfn , low , stk , vdcc , tod , root );
        }
    };
...
/**
 * @brief 随机数生成器 [l1,r1] 和 [l2,r2] 两区间
 * @param l1 左端点区间左端点
 * @param r1 左端点区间右端点
 * @param l2 右端点区间左端点
 * @param r2 右端点区间右端点
...
/**
 * @brief 随机数生成器 [l,r] 数组 [nums] 个数 [limit] 限制
 * @param l 左区间
 * @param r 右区间
 * @param limit 总和上限
 * @param nums 数量
